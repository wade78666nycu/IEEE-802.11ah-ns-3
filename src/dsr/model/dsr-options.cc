...
Ipv4Address
DsrOptions::get_same_channel_Ipv4Address(Ipv4Address channel_addr, Ipv4Address current_ipv4)
{
...
void
DsrOptions::create_ip_to_id_map()
{
...
bool
DsrOptions::ContainAddressAfter(Ipv4Address ipv4Address, Ipv4Address destAddress, std::vector<Ipv4Address>& nodeList)
{
...
{
	NS_LOG_FUNCTION(this);
	std::unordered_set<unsigned int> vec_id;
	for (auto ip : vec)
	{
		vec_id.emplace(GetIDfromIP(ip));
	}

	for (const auto ip_addr : vec2)
	{
		if (vec_id.count(GetIDfromIP(ip_addr)))
		{
			return true;
		}
	}
...
{
	NS_LOG_FUNCTION(this);
	// Remove duplicate ip address from the route if any, should not happen with
	// normal behavior nodes
...
DsrOptions::GetIDfromIP(Ipv4Address address)
{
	...
	for (int32_t i = 0; i < nNodes; ++i) {
		...
		for (unsigned int j = 1; j <= net_card_num; ++j) {
			if (ipv4->GetAddress(j, 0).GetLocal() == address) {
				return i;
			}
		}
	}
...
{
	static TypeId tid =
		TypeId("ns3::dsr::DsrOptionPad1").SetParent<DsrOptions>().SetGroupName("Dsr").AddConstructor<DsrOptionPad1>();
	return tid;
}
...
{
	static TypeId tid =
		TypeId("ns3::dsr::DsrOptionPadn").SetParent<DsrOptions>().SetGroupName("Dsr").AddConstructor<DsrOptionPadn>();
	return tid;
}
...
{
	static TypeId tid =
		TypeId("ns3::dsr::DsrOptionRreq").SetParent<DsrOptions>().SetGroupName("Dsr").AddConstructor<DsrOptionRreq>();
	return tid;
}
...
{
	NS_LOG_FUNCTION(this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t)protocol << isPromisc);
	NS_LOG_DEBUG("In DsrOptionRreq::Process");
	// Fields from IP header
	Ipv4Address srcAddress = ipv4Header.GetSource();
	/*
	 * \ when the ip source address is equal to the address of our own, this is
	 * request packet originated \ by the node itself, discard it
	 */
	if (GetIDfromIP(source) == GetIDfromIP(ipv4Address))
	{
	...
	/*
	 * Get the node associated with the ipv4 address and get several objects from
	 * the node and leave for further use
	 */
	Ptr<Node> node = GetNodeWithAddress(ipv4Address);
	Ptr<dsr::DsrRouting> dsr = node->GetObject<dsr::DsrRouting>();

	Ptr<Packet> p = packet->Copy(); // Note: The packet here doesn't contain the
									// fixed size dsr header
	/*
	...
	{
		NS_LOG_DEBUG("Discard the packet, malformed header since too many ip "
					 "addresses in route");
		m_dropTrace(packet); // call the drop trace to show in the tracing
		return 0;
	}
	...
	/*
	 * Set the number of addresses with the value from peek data and remove the
	 * rreq header
	 */
	...
	// Get the real source address of this request, it will be used when checking
	// if we have received the save route request before or not
	...
	// check whether we have received this request or not, if not, it will save
	// the request in the table for later use, if not found, return false, and
	// push the newly received source request entry in the cache

	// Get the TTL value, this is used to test if the packet will be forwarded or
	// not
	uint8_t ttl = ipv4Header.GetTtl();
	bool dupRequest = false; // initialize the duplicate request check value
	if (ttl)
	{
		// if the ttl value is not 0, then this request will be forwarded, then we
		// need to save it in the source entry
	...
	else
	{
		...
		Ptr<Node> target_node = GetNodeWithAddress(targetAddress);
		unsigned int net_card_num = target_node->GetNDevices() - 1;
        /*
		bool isRouteInCache = false;
		Ptr<Node> target_node = GetNodeWithAddress(targetAddress);
		Ptr<Ipv4> target_ipv4 = target_node->GetObject<Ipv4>();
		unsigned int net_card_num = target_node->GetNDevices() - 1;
		for (unsigned int i = 1; i <= net_card_num; ++i)
		{
			Ipv4Address addr = target_ipv4->GetAddress(i, 0).GetLocal();
			if (dsr->LookupRoute(addr, toPrev))
			{
				isRouteInCache = true;
				break;
			}
        ...
        */
		 ...
		 */
		NS_LOG_DEBUG("The target address over here " << targetAddress << " and the ip address " << ipv4Address
													 << " and the source address " << mainVector[0]);
		if (GetIDfromIP(targetAddress) == GetIDfromIP(ipv4Address))
		{
			NS_LOG_DEBUG("RREQ arrive destination");
			Ipv4Address nextHop; // Declare the next hop address to use
			if (nodeList.size() == 1)
			{
				NS_LOG_DEBUG("These two nodes are neighbors");
				m_finalRoute.clear();
				/// TODO has changed the srcAddress to source, should not matter either
				/// way, check later
				// m_finalRoute.push_back(source);		 // push back the request originator's address
				m_finalRoute.push_back(nodeList.front()); // push back the request originator's address
			...
			else
			{
				...
				auto next_ipv4 = GetNodeWithAddress(nextHop)->GetObject<Ipv4>();
				int32_t next_int = next_ipv4->GetInterfaceForAddress(nextHop);
				next_int = next_int % net_card_num + 1;					 // channel hop
																		 // int32_t next_int = 1;
				nextHop = next_ipv4->GetAddress(next_int, 0).GetLocal(); // get the next hop
				NS_LOG_DEBUG("nextHop after channel hop: " << nextHop);
			}
			...
			// Ipv4Address ipv4Address_main = node->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal();
			// SetRoute(nextHop, ipv4Address_main);

			uint8_t length = rrep.GetLength(); // Get the length of the rrep header
			...
			// dsr->ScheduleInitialReply(newPacket, ipv4Address_main, nextHop, m_ipv4Route);
			/*
			 * Create the route entry to the rreq originator and save it to route
			 * cache, also need to reverse the route
			 */
			// PrintVector(m_finalRoute);
			if (ReverseRoutes(m_finalRoute))
			{
				...
				if (addRoute)
				{
					...
					auto ipv4 = GetNodeWithAddress(nextHop)->GetObject<Ipv4>();
					int next_int = ipv4->GetInterfaceForAddress(nextHop);
					next_int = next_int % net_card_num + 1; // channel hop
					Ipv4Address route_nextHop = ipv4->GetAddress(next_int, 0).GetLocal();
					NS_LOG_DEBUG("The nextHop address " << route_nextHop);
					...
					SetRoute(route_nextHop, ipv4Address);

					// Send the data packet from the send buffer ??
					dsr->SendPacketFromBuffer(sourceRoute, nextHop, protocol);
					// Cancel the route request timer for destination after sending the
					// data packet
		...
		/*
		 * (ii) or it has an active route to the destination, send reply based on
		 * request header and route cache, need to delay based on a random value
		 * from d = H * (h - 1 + r), which can avoid possible route reply storm.
		 * Also, verify if two vectors do not contain duplicates (part of the route
		 * to the destination from route cache and route collected so far). If so,
		 * do not use the route found and forward the route request.
		 */
		 else if (isRouteInCache && !areThereDuplicates)
		{
			...
			/**
			 * push back the route vector we found in our route cache to destination,
			 * including this node's address
			 */
			...
			/*
			 * First remove the duplicate ip address to automatically shorten the
			 * route, and then reversely search the next hop address
			 */
			// Set the route
			SetRoute(nextHop, ipv4Address);
		...
		else
		{
			...
			// NS_LOG_DEBUG("main front: " << mainVector.front() << " source: " <<
			// source); NS_ASSERT(mainVector.front() == source);
			...
			uint8_t ttl = ipv4Header.GetTtl();
			/*
			 * Decrease the TTL value in the packet tag by one, this tag will go to ip
			 * layer 3 send function and drop packet when TTL value equals to 0
			 */
			...
			if (ttl)
			{
				...
				auto ipv4 = GetNodeWithAddress(ipv4Address)->GetObject<Ipv4>();
				int next_int = ipv4->GetInterfaceForAddress(ipv4Address);
				dsr->send_interface = next_int % net_card_num + 1; // channel hop
				NS_LOG_DEBUG("call ScheduleInterRreq, send_interface: " << dsr->send_interface);
...
{
	static TypeId tid =
		TypeId("ns3::dsr::DsrOptionRrep").SetParent<DsrOptions>().SetGroupName("Dsr").AddConstructor<DsrOptionRrep>();
	return tid;
}
...
{
	NS_LOG_FUNCTION(this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t)protocol << isPromisc);
	NS_LOG_DEBUG("In DsrOptionRrep::Process");
	...
	unsigned int node_id = GetIDfromIP(ipv4Address);
	unsigned int net_card_num = node->GetNDevices() - 1;
	if (GetIDfromIP(targetAddress) == node_id)
	{
		NS_LOG_DEBUG("print nodeList:");
		PrintVector(nodeList);
		RemoveDuplicates(nodeList); // This is for the route reply from intermediate
									// node since we didn't remove duplicate there
		if (nodeList.empty())
		{
			NS_LOG_DEBUG("The route we have contains 0 entries");
			return 0;
		}
		NS_LOG_DEBUG("RREP arrive destination");
		/**
		 * Get the destination address for the data packet, which is the last
		...
		DsrRouteCacheEntry toDestination(/*ip=*/nodeList,
										 ...
										 /*exp=*/ActiveRouteTimeout);
		// 為了要讓 cache 可以 work，
		// 當初在 dsr-routing.cc 的 SendInitialRequest 把 source ip 退回一個，
		// 原本的檢查 NS_ASSERT(nodeList.front() == ipv4Address);
		// 記得要改成檢查 source ip 加一個回來
		auto src_ipv4 = node->GetObject<Ipv4>();
		int32_t src_int = src_ipv4->GetInterfaceForAddress(nodeList.front());
		src_int = src_int % net_card_num + 1;								// channel hop
		Ipv4Address src_addr = src_ipv4->GetAddress(src_int, 0).GetLocal(); // get the next hop
		NS_ASSERT(src_addr == ipv4Address);
		bool addRoute = false;
		if (dsr->IsLinkCache())
		{
		...
		if (addRoute)
		{
			...
			sourceRoute.SetSalvage(0);
			// 搜尋 nextHop 的時候一樣要記得搜尋的 nodeList.front()
			Ipv4Address nextHop = SearchNextHop(nodeList.front(), nodeList);
			NS_LOG_DEBUG("The nextHop address " << nextHop);
			if (nextHop == "0.0.0.0")
			{
				dsr->PacketNewRoute(dsrP, ipv4Address, dst, protocol);
			...
			// PrintVector(nodeList);
			// SetRoute(nextHop, src_addr);
			SetRoute(nextHop, ipv4Address);
			// Cancel the route request timer for destination
			dsr->CancelRreqTimer_with_all_ip(dst, true);
			/**
			 * Schedule the packet retry
			 */
			dsr->SendPacketFromBuffer(sourceRoute, nextHop, protocol);
	...
	{
		uint8_t length = rrep.GetLength() - 2; // The get length - 2 is to get aligned for the
		...
		PrintVector(nodeList);
		/*
		 * This node is only an intermediate node, but it needs to save the possible
		 * route to the destination when cutting the route
		 */
		std::vector<Ipv4Address> routeCopy = nodeList;

		Ptr<Ipv4> ipv4 = node->GetObject<Ipv4>();
		int interface_id = ipv4->GetInterfaceForAddress(ipv4Address);
		if (--interface_id <= 0)
		{
			interface_id = net_card_num;
		}
		Ipv4Address addr_in_route = ipv4->GetAddress(interface_id, 0).GetLocal();
		std::vector<Ipv4Address> cutRoute = CutRoute(addr_in_route, nodeList);
		...
		if (cutRoute.size() >= 2)
		{
			...
			DsrRouteCacheEntry toDestination(/*ip=*/cutRoute,
											 /*dst=*/dst,
											 /*exp=*/ActiveRouteTimeout);
			NS_ASSERT(cutRoute.front() == addr_in_route);
			bool addRoute = false;
			if (dsr->IsLinkCache())
			{
				addRoute = dsr->AddRoute_Link(nodeList, addr_in_route);
			}
			else
			{
				addRoute = dsr->AddRoute(toDestination);
		 ...
		 */
		Ipv4Address nextHop = ReverseSearchNextHop(addr_in_route, routeCopy);
		PrintVector(routeCopy);
		NS_LOG_DEBUG("The nextHop address " << nextHop << " and the source in the route reply " << source);
		NS_ASSERT(GetIDfromIP(routeCopy.back()) == GetIDfromIP(source));

		// Set the route entry we will use to send reply
		Ipv4Address nextHop_addr = get_same_channel_Ipv4Address(addr_in_route, nextHop);
		SetRoute(nextHop_addr, addr_in_route);

		// This part add dsr routing header to the packet and send reply
		DsrRoutingHeader dsrRoutingHeader;
		dsrRoutingHeader.SetNextHeader(protocol);

		length = rrep.GetLength(); // Get the length of the rrep header excluding
								   // the type header
		...
		Ptr<Packet> newPacket = Create<Packet>();
		newPacket->AddHeader(dsrRoutingHeader);
		dsr->SendReply(newPacket, addr_in_route, nextHop_addr, m_ipv4Route);
		isPromisc = false;
	}
...
{
	static TypeId tid =
		TypeId("ns3::dsr::DsrOptionSR").SetParent<DsrOptions>().SetGroupName("Dsr").AddConstructor<DsrOptionSR>();
	return tid;
}
...
DsrOptionSR::Process(Ptr<Packet> packet,
					 ...
					 Ipv4Address promiscSource)
{
	NS_LOG_FUNCTION(this << packet << dsrP << ipv4Address << source << ipv4Address << ipv4Header << (uint32_t)protocol
						 << isPromisc);
	NS_LOG_DEBUG("In DsrOptionSR::Process");
	Ptr<Packet> p = packet->Copy();
	// Get the number of routers' address field
	uint8_t buf[2];
	p->CopyData(buf, sizeof(buf));
	...
	if (isPromisc)
	{
		...
		if (destAddress != destination)
		{
			...
			for (int32_t i = 0; i < nNodes; ++i)
			{
				...
				// The source and destination addresses here are the real source and
	...
	else
	{
		...
		optionType = *(data);
		/// When the option type is 160, means there is ACK request header after the
		/// source route, we need to send back acknowledgment
		if (optionType == 160)
		{
			 ...
			 * If the node list is not empty, find the previous hop from the node
			 * list, otherwise, use srcAddress
			 */
		...
		/*
		 * After send back ACK, check if the segments left value has turned to 0 or
		 * not, if yes, update the route entry and return header length
		 */
		if (segsLeft == 0)
		{
		...
		if (segsLeft > numberAddress) // The segmentsLeft field should not be larger
									  // than the total number of ip addresses
		{
			NS_LOG_LOGIC("Malformed header. Drop!");
			m_dropTrace(packet);
			return 0;
		...
		Ipv4Address channel_addr = get_same_channel_Ipv4Address(nextAddress, ipv4Address);
		SetRoute(nextAddress, channel_addr);
...
{
	static TypeId tid =
		TypeId("ns3::dsr::DsrOptionRerr").SetParent<DsrOptions>().SetGroupName("Dsr").AddConstructor<DsrOptionRerr>();
	return tid;
}
					   ...
					   Ipv4Address promiscSource)
{
	...
	if (errorType == 1) // unreachable ip address
	{
		...
		NS_LOG_DEBUG("The error source is " << rerrUnreach.GetErrorDst() << "and the unreachable node is "
											<< unreachAddress);
		/*
		...
		/*
		 * Delete all the routes including the unreachable node address from the
		 * route cache
		 */
		Ptr<Node> node = GetNodeWithAddress(ipv4Address);
	...
	else
	{
		...
		// uint32_t serialized = DoSendError (p, rerrUnsupport, rerrSize,
						   ...
						   uint8_t protocol)
{
	...
	// Here remove the source route header and schedule next hop error
	// transmission
						 ...
						 Ipv4Address promiscSource)
{
	NS_LOG_FUNCTION(this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t)protocol << isPromisc);
	/*
	 * Current implementation of the ack request header processing is coded in
	 * source route header processing
	 */
	/*
	...
	p->RemoveHeader(ackReq);
	/*
	 * Get the node with ip address and get the dsr extension and reoute cache
	 * objects
	 */
	Ptr<Node> node = GetNodeWithAddress(ipv4Address);
	Ptr<dsr::DsrRouting> dsr = node->GetObject<dsr::DsrRouting>();
...
{
	static TypeId tid =
		TypeId("ns3::dsr::DsrOptionAck").SetParent<DsrOptions>().SetGroupName("Dsr").AddConstructor<DsrOptionAck>();
	return tid;
}
					  ...
					  Ipv4Address promiscSource)
{
	...
	/*
	 * Get the node with ip address and get the dsr extension and route cache
	 * objects
	 */
	Ptr<Node> node = GetNodeWithAddress(ipv4Address);
	Ptr<dsr::DsrRouting> dsr = node->GetObject<dsr::DsrRouting>();
