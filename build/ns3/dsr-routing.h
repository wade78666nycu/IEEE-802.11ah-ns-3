...
class DsrRouting : public IpL4Protocol
{
  public:
	...
	bool m_enable_global_broadcast;
	bool m_use_gradpc; // whether using gradpc or not, set to true if using grad-pc
					   // std::vector<std::unordered_set<uint32_t>> m_neighbor_list;
	unsigned int send_interface;

	/**
	 * \brief Send broadcast packet to single hop neighbors
	 * \param packet the original packet
	 * \param source address
	 */
	void SendBroadcast(Ptr<Packet> packet, Ipv4Address source);

	// 改用目的地node的所有網卡ip找一遍
	// return (true, matching_destination_address) if found
	std::tuple<bool, Ipv4Address> Lookup_destination_route_with_all_ip(Ipv4Address destination,
																	   DsrRouteCacheEntry& toDst);
	// debug print function
	void Print_sendBuffer(DsrSendBuffer sendBuffer);

	// get the Ipv4Address that use the same channel as nextHop
	Ipv4Address get_same_channel_Ipv4Address(Ipv4Address channel_addr, Ipv4Address current_ipv4);

	// search for nextHop with all the ip the node have instead of just using
	// m_mainAddress
	Ipv4Address SearchNextHop_with_all_ip(Ipv4Address ipv4Address, std::vector<Ipv4Address>& vec);
	void CancelLinkPacketTimer_with_all_ip(DsrMaintainBuffEntry& mb);
	void CancelRreqTimer_with_all_ip(Ipv4Address dst, bool isRemove);
	// ↑↑↑↑↑↑  alan's modification  ↑↑↑↑↑↑
	...
	/**
	 * \brief Delete all the routes which includes the link from next hop address
	 * that has just been notified as unreachable. See also
	 * DsrRouteCache::DeleteAllRoutesIncludeLink
	 *
	...
	/**
	 * \brief Update route cache entry if it has been recently used and
	 * successfully delivered the data packet. See also
	 * DsrRouteCache::UpdateRouteEntry
	 *
	...
	/**
	 * Find the source request entry in the route request queue, return false if
	 * not found. See also DsrRreqTable::FindSourceEntry
	 *
	...
	/**
	 * \brief Check the send buffer of packets with route when send buffer timer
	 * expire.
	 */
	void CheckSendBuffer();
	/**
	 * \brief When route vector corrupted, originate a new packet, normally not
	 * happening. \param packet to route \param source address \param destination
	 * address \param protocol number
	 */
	...
	/**
	 * \brief This function is responsible for sending error packets in case of
	 * break link to next hop \param unreachNode unreachable node \param
	 * destination address \param originalDst address \param salvage packet flag
	...
	/**
	 * \brief This function is responsible for forwarding error packets along the
	 * route \param rerr unreachable header \param sourceRoute source routing
	 * header \param nextHop IP address of next hop \param protocol number \param
	 * route IP route
	 */
	void ForwardErrPacket(DsrOptionRerrUnreachHeader& rerr,
						  ...
						  Ptr<Ipv4Route> route);
	/**
	 * \brief This function is called by higher layer protocol when sending
	 * packets \param packet to send \param source IP address \param destination
	 * IP address \param protocol number \param route IP route
	 */
	void Send(Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination, uint8_t protocol, Ptr<Ipv4Route> route);
	/**
	 * \brief This function is called to add ack request header for network
	 * acknowledgement \param packet for ack req \param nextHop IP address of the
	 * next hop \return ack ID
	 */
	...
	/**
	 * \brief This function is called to schedule sending packets from the network
	 * queue \param priority for sending
	 */
	void Scheduler(uint32_t priority);
	/**
	 * \brief This function is called to schedule sending packets from the network
	 * queue by priority \param priority schedule \param continueWithFirst use all
	 * priorities
	 */
	void PriorityScheduler(uint32_t priority, bool continueWithFirst);
	/**
	 * \brief This function is called to increase the retransmission timer for
	 * data packet in the network queue
	 */
	...
	/**
	 * \brief This function is responsible for sending out data packets when have
	 * route, if no route found, it will cache the packet and send out route
	 * requests \param sourceRoute source route \param nextHop next hop IP address
	 * \param protocol number
	 */
	...
	/**
	 * \brief Cancel the network packet retransmission timer for a specific
	 * maintenance entry \param mb maintain buffer entry
	 */
	void CancelNetworkPacketTimer(DsrMaintainBuffEntry& mb);
	/**
	 * \brief Cancel the passive packet retransmission timer for a specific
	 * maintenance entry \param mb maintain buffer entry
	 */
	void CancelPassivePacketTimer(DsrMaintainBuffEntry& mb);
	/**
	 * \brief Cancel the link packet retransmission timer for a specific
	 * maintenance entry \param mb maintain buffer entry
	 */
	void CancelLinkPacketTimer(DsrMaintainBuffEntry& mb);
	/**
	 * \brief Cancel the packet retransmission timer for a all maintenance entries
	 * with nextHop address \param nextHop next hop IP address \param protocol
	 * number
	 */
	...
	/**
	 * \brief Schedule the packet retransmission based on link-layer
	 * acknowledgment \param mb maintenance buffer entry \param protocol the
	 * protocol number
	 */
	...
	/**
	 * \brief Schedule the packet retransmission based on network layer
	 * acknowledgment \param mb maintenance buffer entry \param isFirst see if
	 * this is the first packet retry or not \param protocol the protocol number
	 */
	void ScheduleNetworkPacketRetry(DsrMaintainBuffEntry& mb, bool isFirst, uint8_t protocol);
	/**
	 * \brief This function deals with packet retransmission timer expire using
	 * link acknowledgment \param mb maintenance buffer entry \param protocol the
	 * protocol number
	 */
	void LinkScheduleTimerExpire(DsrMaintainBuffEntry& mb, uint8_t protocol);
	/**
	 * \brief This function deals with packet retransmission timer expire using
	 * network acknowledgment \param mb maintenance buffer entry \param protocol
	 * the protocol number
	 */
	void NetworkScheduleTimerExpire(DsrMaintainBuffEntry& mb, uint8_t protocol);
	/**
	 * \brief This function deals with packet retransmission timer expire using
	 * passive acknowledgment \param mb maintenance buffer entry \param protocol
	 * the protocol number
	 */
	void PassiveScheduleTimerExpire(DsrMaintainBuffEntry& mb, uint8_t protocol);
	/**
	 * \brief Forward the packet using the route saved in the source route option
	 * header \param packet The packet \param sourceRoute Source route saved in
	 * option header \param ipv4Header IPv4 Header \param source source address
	...
	void SendGratuitousReply(Ipv4Address replyTo,
							 ...
							 uint8_t protocol);
	/**
	 * Send the route reply back to the request originator with the cumulated
	 * route
	 *
	 * \param packet the original packet
	...
	/**
	 * this is a generating the initial route reply from the destination address,
	 * a random delay time [0, m_broadcastJitter] is used before unicasting back
	...
	/**
	 * Schedule the cached reply to a random start time to avoid possible route
	 * reply storm
	 *
	 * \param packet the original packet
	 * \param source IPv4 address of the source (i.e. request originator)
	...
	void ScheduleCachedReply(Ptr<Packet> packet,
							 ...
							 double hops);
	/**
	 * Send network layer acknowledgment back to the earlier hop to notify the
  ...
  protected:
	/*
	 *    * This function will notify other components connected to the node that
	 * a new stack member is now connected
	 *       * This will be used to notify Layer 3 protocol of layer 4 protocol
	 * stack to connect them together.
	 *          */
  ...
  private:
	 ...
	 * \param packetType The dsr packet type, 0 is for control packet, 1 for data
	 * packet \return true if the packet was processed, false otherwise
	 */
	...
	uint32_t m_passiveRetries; ///< # of retries have been sent for passive
							   ///< acknowledgment
	...
	Time m_nodeTraversalTime; ///< Time estimated for packet to travel between two
							  ///< nodes

	uint32_t m_maxSendBuffLen; ///< The maximum number of packets that we allow a
							   ///< routing protocol to buffer.

	Time m_sendBufferTimeout; ///< The maximum period of time that a routing
	...
	uint32_t m_maxEntriesEachDst; ///< Max number of route entries to save for
								  ///< each destination
	...
	uint32_t m_tryPassiveAcks; ///< Maximum number of packet transmission using
							   ///< passive acknowledgment

	Time m_linkAckTimeout; ///< The timeout value for link acknowledge

	uint32_t m_tryLinkAcks; ///< Maximum number of packet transmission using link
							///< acknowledgment
	...
	Time m_retransIncr; ///< the increase time for retransmission timer when face
						///< network congestion
	...
	Ptr<dsr::DsrRouteCache> m_routeCache; ///< A "drop-front" queue used by the routing layer to cache
										  ///< routes found.
	...
	std::unordered_map<std::string, uint32_t> ip_to_id_map;
};
} /* namespace dsr */
} /* namespace ns3 */
