...
class DsrOptions : public Object
{
  public:
	 ...
	 *
	 * \param ipv4Address the IPv4 address of the node we are looking for its next
	 * hop address \param vec List of IPv4 addresses \return the next hop address
	 * if found
	 */
	 ...
	 *
	 * \param ipv4Address the IPv4 address of the node we are looking for its next
	 * hop address \param vec List of IPv4 addresses \return the previous next hop
	 * address if found
	 */
	 ...
	 *
	 * \param ipv4Address the IPv4 address of the node we are looking for its next
	 * two hop address \param vec List of IPv4 addresses \return the previous next
	 * two hop address if found
	 */
  ...
  protected:
	...
	/**
	 * The receive trace back, only triggered when final destination receive data
	 * packet
	 */
  ...
  private:
	...
	std::unordered_map<std::string, int> ip_to_id_map;
};
